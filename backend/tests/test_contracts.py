...
def test_runs_do_not_share_mutable_data():
    ...
    first["errors"].append(
        AgentError(code="TIMEOUT", message="Timed out", source="analysis")
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {
            "status": "success",
            "error": {"code": "TIMEOUT", "message": "Timeout", "source": "planner"},
        },
    ],
)
...
def test_clarification_and_error_responses():
    ...
    failure = AgentResponse[PlannerUpdates](
        ...
        updates=PlannerUpdates(),
        error=AgentError(
            code="MISSING_COLUMN", message="Unknown column", source="planner"
        ),
    )
...
def test_critic_success_can_request_revision():
    response = AgentResponse[CriticUpdates](
        status="success",
        updates=CriticUpdates(
            critique=Critique(decision="revise", required_changes=["Check sample"])
        ),
    )
