...
class AgentResponse(ContractModel, Generic[UpdatesT]):
    ...
    def validate_status(self) -> "AgentResponse[UpdatesT]":
        ...
        if (self.status == "needs_clarification") != (self.clarification_question is not None):
            raise ValueError("Only needs_clarification must include a clarification_question")
        return self
